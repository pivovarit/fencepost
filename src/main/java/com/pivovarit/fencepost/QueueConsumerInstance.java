...
final class QueueConsumerInstance implements QueueConsumer {
    ...
    private void consumeLoop() {
        while (!closed) {
            if (Thread.interrupted()) {
                logger.warn("consumer thread for queue '{}' found its interrupt flag set outside close(), most likely re-asserted by the handler; clearing it and continuing",
                  queueName);
            }
            try {
                ...
                if (closed || Thread.currentThread().isInterrupted()) {
                    continue; // loop head exits, or clears a handler-set flag and retries, without reporting or backing off
                }
    ...
    private void ackIfActive(Message msg) {
        ...
        switch (state) {
            case ACTIVE -> am.ack();
            case CLOSED -> logger.debug("handler released message {} on queue '{}' via close(); it becomes eligible for redelivery (or dead-lettering if maxDeliveries is reached) once the visibility timeout expires",
              am.id(), queueName);
